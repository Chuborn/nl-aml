...
# For Linux interactive keyboard input
try:
    import tty
    import termios
except ImportError:
...
class NL_Checker:
    ...
    def get_key(self) -> str:
        """
        Linux-compatible getch equivalent for interactive menus.
        """
        if tty is None or termios is None:
            # Fallback to simple input for safety
            return input()
        ...
        try:
            ...
            ch = sys.stdin.read(1)
            # Check for arrow keys (Escape sequence \x1b[)
            if ch == '\x1b':
                ch += sys.stdin.read(2)
        finally:
    ...
    def setup_results(self):
        ...
        self.results_dir = os.path.join("results", f"nl_linux_{now}")
        ...
        self.ongoing_root_file = f"ongoing-nl-linux-{now}.txt"
    ...
    def select_file(self):
        ...
        while True:
            os.system('clear')
            print(f"{ColorCyan}{ColorBold}NL CHECKER (NotLetters LINUX) - BY Chuborn{ColorReset}")
    ...
    def extract_emails(self, filename):
        ...
        with open(filename, 'r', encoding='utf-8', errors='ignore') as f:
            for line in f:
                ...
                # 2. UKN Format Check (Generic 2 emails)
                emails = re.findall(email_pattern, line)
                if len(emails) >= 2:
                    email2 = emails[1]
                    ukn_match = re.search(re.escape(email2) + r':([^\s|]+)', line)
                    if ukn_match:
    ...
    def check_account(self, full_line, email, password):
        ...
        try:
            ...
            # 3. Fetch mail content and categorize
            full_body_text = ""
            newest_timestamp = 0
